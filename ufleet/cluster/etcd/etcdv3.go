...
func (le *LEtcd) Init(IPs []string) error {
	...
	for i := range IPs{
		endpoints = append(endpoints, fmt.Sprintf("http://%s:%d", IPs[i], config.GEtcdListenPort))
	}
...
//	}
//
//	_, err = le.client.MemberAdd(le.ctx, []string{fmt.Sprintf("http://%s:%d", IP, config.GEtcdPeerPort)})
//	return err
//}
...
//	}
//
//	retryTimes := config.GTimesOfCheckEtcd
//
